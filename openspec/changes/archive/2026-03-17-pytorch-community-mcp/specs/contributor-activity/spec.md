...
- **THEN** system returns a unified summary including: GitHub PRs/issues/comments by ezyang, Slack messages from @ezyang, and Discourse posts by ezyang, all within the specified time range

#### Scenario: Partial platform data
- **WHEN** one platform (e.g., Slack) is not configured or returns an error
- **THEN** system returns available data from other platforms with a note indicating which platform was unavailable
...
- **WHEN** contributor's GitHub username matches their Slack display name and Discourse username
